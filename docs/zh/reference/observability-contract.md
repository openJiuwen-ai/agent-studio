...
4. 当前服务能够使用自身错误目录和 i18n 资源生成安全的三段本地化文案（`message`/`reason`/`suggestion`）；`error_code` 是独立稳定字段，不属于 i18n 文案；
...
| `i18n_key` | string | 是 | 指向三段本地化安全文案（`message`/`reason`/`suggestion`）；`error_code` 是独立稳定字段，不属于 i18n 文案；支持语言必须具有完整资源 |
