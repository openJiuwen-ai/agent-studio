...
在openJiuwen平台中，插件是扩展工作流与智能体能力的关键方式。平台支持四种插件类型：RESTful API云侧插件、MCP服务端插件、本地自定义服务端，以及代码插件。每个插件可包含多个工具，且这些工具必须属于同一域名。每个工具对应独立的API。

# 添加插件与工具

在openJiuwen中，您可以通过以下三种方式添加插件：

`云侧插件：基于已有服务创建`：连接已有的服务，可以是您在本地通过[后台运行插件服务](#后台运行插件服务)运行的自定义插件服务，也可以是符合RESTful格式的外部API。

`MCP服务端插件`：连接MCP（模型上下文协议）服务端，使用五种传输协议之一与MCP兼容服务端建立连接，服务端暴露的所有工具将被自动发现和注册。
...
openJiuwen平台支持连接**MCP（模型上下文协议）**服务端。MCP是一个开放标准，允许AI应用通过统一协议连接外部数据源和工具。安装MCP插件后，该服务端暴露的所有工具将被自动发现并可供智能体和工作流使用。

### 支持的传输类型

| 传输类型 | 说明 | 必填字段 |
|:---:|:---|:---:|
| STDIO | 通过标准输入/输出启动本地MCP服务端进程 | 服务端脚本文件路径 |
| SSE | 使用服务器发送事件（Server-Sent Events）连接MCP服务端 | 服务端URL |
| Streamable HTTP | 使用可流式HTTP连接MCP服务端 | 服务端URL |
| OpenAPI | 连接兼容OpenAPI的MCP端点 | 规范文件路径或URL |
| Playwright | 通过Playwright连接基于浏览器的MCP服务端 | 服务端URL |

### 操作步骤

1. 进入平台左侧导航栏的**插件管理**模块。

2. 单击**安装插件**按钮，选择**MCP服务端插件**。
...
3. 填写插件信息：
   ...
   | 插件名称 | 插件的显示名称，用于识别插件。 |
   | 插件描述 | 插件用途的简短描述。 |
   | 插件详情 | Markdown格式的详细说明（可选）。 |
   | 传输类型 | 连接MCP服务端使用的通信协议：STDIO、SSE、Streamable HTTP、OpenAPI或Playwright。 |
   | 服务端URL / 文件位置 | MCP服务端的URL（适用于SSE、Streamable HTTP、Playwright）或服务端脚本/规范文件路径（适用于STDIO、OpenAPI）。 |

4. 单击**创建插件**。插件记录立即保存，此阶段不会连接MCP服务端。

### 发现工具

创建插件后，需要手动触发工具发现：

1. 在插件卡片上单击**配置**，进入插件配置页面。

2. 切换至**工具设置**选项卡。

3. 单击**发现工具**按钮。

   平台将连接MCP服务端，获取所有可用工具并保存到数据库。如果之前已发现过工具，旧工具将被删除，并替换为最新发现的工具集。

4. 发现完成后，工具列表中将显示所有已发现的工具，每个工具自动填充其名称、描述和参数列表。

> **注意：** MCP工具为**只读**模式，不能手动添加、编辑或删除单个工具。MCP服务端发生变化后，可随时再次单击**发现工具**以刷新工具列表。

### 授权请求头

如果MCP服务端需要身份验证，请将凭证作为插件参数添加：

1. 在插件配置页面切换至**插件参数**选项卡。

2. 单击**添加参数**，按如下配置：

   | 配置项 | 值 |
   |:---:|:---|
   | 参数名称 | HTTP请求头名称，例如 `Authorization` |
   | 传入方式 | `Header` |
   | 非运行时参数 | 启用，并将令牌设置为**默认值**，例如 `Bearer YOUR_TOKEN` |

平台在工具发现和运行时工具执行过程中，会自动将该请求头包含在所有与MCP服务端的连接中。

### 在智能体和工作流中使用MCP工具

工具发现完成且状态为**启用**后，可在智能体和工作流中使用，使用方式与其他插件工具完全相同。在工具面板中选择对应插件及工具，输入参数由智能体或工作流节点在运行时填写。
