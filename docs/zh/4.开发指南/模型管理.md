...
5. 在弹出的配置界面中，设置相关参数。<br>
   ...
   **参数说明**
   ...
   | API密钥      | 通用          | 模型服务的身份验证凭证（API Key），通常可以从模型服务的控制台获取，填写后将自动加密存储。 <br>示例：`sk-xxxx`<br>注意：如果不需要 API 密钥（例如在本地 Ollama 中），请使用通配符 `*`                                                       |
   | 基础服务地址     | 通用          | 模型服务的API调用地址（Base URL）示例：`https://api.openai.com/v1` (注意：像 Ollama 这样的服务提供商需要 `/v1/` 后缀。)                                                                    |
