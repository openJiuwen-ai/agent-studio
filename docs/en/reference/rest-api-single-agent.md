...
11. [Batch Query Agent Version References](#11-batch-query-agent-version-references)
12. [Batch Delete Agent Versions](#12-batch-delete-agent-versions)
...
## 11. Batch Query Agent Version References

**Description**

This API is used to query the reference count of each version of a specified agent application, with optional filtering by version ID. The reference count includes agents and workflows in the current workspace that directly reference the version, as well as usages of the version through sharing. Versions shared to the asset plaza are marked with `is_shared` as `true`, and the latest version is marked with `is_latest` as `true`.

**URI**

```
GET /v1/{project_id}/agent-manager/agents/{agent_id}/versions/references?workspace_id={workspace_id}
```
...
```
GET /v1/{project_id}/agent-manager/agents/{agent_id}/versions/references?workspace_id={workspace_id} HTTP/1.1
Host: api.example.com
X-Auth-Token: {token}
```
...
|-----------|------|-------------|
| version_id | String | Version ID |
| error_code | String | Error code, e.g., `SHARE_RESOURCE_CANNOT_BE_DELETE_DIRECTLY` (shared version cannot be deleted directly), `AGENT_VERSION_NOT_EXIST` (version does not exist) |
| error_msg | String | Error message |
...
```
POST /v1/{project_id}/agent-manager/agents/{agent_id}/versions/batch-delete?workspace_id={workspace_id} HTTP/1.1
Host: api.example.com
Content-Type: application/json
X-Auth-Token: {token}

{
  "version_ids": ["1787800848138", "1787901244768", "1788000000000"]
}
```
...
```json
{
  ...
  "failed": [
    ...
    {
      "version_id": "1788000000000",
      "error_code": "AGENT_VERSION_NOT_EXIST",
      "error_msg": "Agent version does not exist"
    }
  ]
}
```
