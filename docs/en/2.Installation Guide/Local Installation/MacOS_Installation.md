...
2. **Start the sandbox service**
   ...
   On macOS, only **local** execution mode is supported: code runs directly on the host. Using `sandbox_server/sandbox/.env.example` as a reference, create a `.env` file under `sandbox_server/sandbox`. Example:
