...
3. 获取以下信息：
   - `ENDPOINT` — 服务访问地址（默认 `http://localhost:8080`）
   - `PROJECT_ID` — 项目 ID（默认 `0`，Simple 鉴权下使用默认项目即可）
   - `AGENT_ID` — 智能体应用 ID（平台「开发中心 > 智能体管理」中创建并发布后，从应用详情页获取）

## 鉴权说明

默认使用 **Simple 鉴权**（`AUTH_SSO_VALIDATE_URL` 留空时自动启用），通过 `X-Auth-Token` Header 传递 Token，格式为 `userId|projectId`。
...
> 如启用了 SSO 鉴权（配置了 `AUTH_SSO_VALIDATE_URL`），请将 `AUTH_TOKEN` 替换为实际 SSO Token，并传入 `Access-Token` Header。

## 运行

```bash
# 仅需提供 AGENT_ID，其余使用默认值
AGENT_ID=your_agent_id bash quick-chat.sh "什么是 AgentStudio？"
```
