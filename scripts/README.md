...
- 旧版本 0.1.4：`.envs/env.deploy.<实例ID>` 与 `.envs/env.runtime.<实例ID>`
...
## 脚本报错 ./global_vars.sh: line 11: ENV_DIR: unbound variable 解决方案

**问题现象**
部署工具在运行过程中，提示变量 ENV_DIR 未定义而异常退出，终端输出如下错误信息：

```
./global_vars.sh: line 11: ENV_DIR: unbound variable
```
...
## 遇到Please define IP in .env.custom怎么办？

**问题现象**
部署工具在运行的过程中报如下错误，导致部署流程无法正常启动:

```
❌ Please define IP in .env.custom
```

**问题原因**
该错误表示部署工具的核心配置文件 .env.custom 中未定义 IP 配置项，工具无法获取运行部署工具的本机 IP 地址，无法完成服务部署。
