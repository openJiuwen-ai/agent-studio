 ...
 * 复用下层构件 {@code buildModelExportEntity} / {@code UrlCheckUtils} / {@code ImportRsp}。
 * 导出走 JSONL；导入落库用 {@code ModelServiceManager} 保留跨环境 id。
 *
 * <p>鉴权 MASKED only（agent-studio 无加密 SPI）：导出时 authInfo 两处均置为空格占位，
 * 导入后用户在目标环境重新配置。
 */
public interface IModelImportExportService {

    /**
     * 批量导出模型为 JSONL（供应商+模型，includeProvider=true）。委托下行。
     *
    ...
    /**
     * 批量导出模型为 JSONL，按 {@code includeProvider} 区分两种模式：
     * <ul>
    ...
    /**
     * 导入预检：逐行解析+冲突检测+URL/占位符校验，<b>不落库</b>。委托下行（targetProviderId=null）。
     */
    ...
    /**
     * 导入落库：逐行解析，按冲突策略 SKIP/COVER 落库，保留导入 id（跨环境一致）。委托下行。
     */
