        ...
        try (Connection conn = dataSource.getConnection()) {
            ...
            if (url != null) {
                if (url.contains("opengauss")) {
                    log.info("JdbcUrlDatabaseIdProvider: detected openGauss (MySQL compatible mode), mapping to mysql");
                    return "mysql";
                }
                if (url.contains("postgresql")) {
                    log.info("JdbcUrlDatabaseIdProvider: detected PostgreSQL");
                    return "postgres";
                }
                if (url.contains("mysql") || url.contains("mariadb")) {
                    log.info("JdbcUrlDatabaseIdProvider: detected MySQL");
                    return "mysql";
                }
            }
