...
class _FakeTracerManager:
    ...
    async def on_plugin_start(self, inputs): pass
    async def on_plugin_end(self, *a, **kw): pass
    async def on_plugin_error(self, error): pass
...
class TestRestfulApiAinvokeFinalBoundary(unittest.IsolatedAsyncioTestCase):
    ...
    def _make_api():
        ...
        api.request_params_creator.create = lambda inputs, **kw: type("RP", (), {
...
class _FakeTracerManager:
    async def on_plugin_start(self, inputs): pass
    async def on_plugin_end(self, content): pass
    async def on_plugin_error(self, error): pass
...
class TestMcpStreamableHttpSendBoundary(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_streamable_http_no_correlation_at_send(self):
        ...
        with mock.patch("jiuwen.plugin.models.mcpapi.TraceManager") as TM,              mock.patch.object(api, "replace_mcp_headers_extra", evil_hook),              mock.patch.object(api, "_validate_auth_hook_function", lambda rp: None),              mock.patch("jiuwen.extension.wrapper.customer_header_inject.inject_customer_headers_to_mcp", lambda rp: None),              mock.patch("jiuwen.plugin.models.mcpapi.streamablehttp_client",
                        lambda *a, **kw: _FakeMcpCtx(captured, kw)),              mock.patch("jiuwen.plugin.models.mcpapi.ClientSession",
...
class TestMcpSseSendBoundary(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_sse_no_correlation_at_send(self):
        ...
        with mock.patch("jiuwen.plugin.models.mcpapi.TraceManager") as TM,              mock.patch.object(api, "replace_mcp_headers_extra", evil_hook),              mock.patch.object(api, "_validate_auth_hook_function", lambda rp: None),              mock.patch("jiuwen.extension.wrapper.customer_header_inject.inject_customer_headers_to_mcp", lambda rp: None),              mock.patch("jiuwen.plugin.models.mcpapi.sse_client",
                        lambda *a, **kw: _FakeMcpSseCtx(captured, kw)),              mock.patch("jiuwen.plugin.models.mcpapi.ClientSession",
                        lambda *a: _FakeClientSession()), \
             mock.patch.object(api, "_transform_result", lambda r: []):
            TM.generate_manager.return_value = _FakeTracerManager()
            await api.ainvoke({})
