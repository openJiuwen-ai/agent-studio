...
class TestNoExceptionConfig:
    def test_no_configs(self):
        node = {"type": "jiuwen.mcp"}
        assert _parse_exception_config(node) is None

    def test_configs_without_exception_process(self):
        node = {"type": "jiuwen.mcp", "configs": {"other_field": "value"}}
        assert _parse_exception_config(node) is None

    def test_exception_process_is_none(self):
        node = _make_node(exception_process=None)
        assert _parse_exception_config(node) is None

    def test_exception_process_is_empty_dict(self):
...
class TestMcpOutputsSchemaEmpty:
    """MCP 节点的 outputs_schema 必须置空，不使用 _convert_schema。"""

    def test_mcp_node_outputs_schema_is_empty(self):
        node = _make_node(
            node_type="jiuwen.mcp",
    ...
    def test_flow_mcp_node_outputs_schema_is_empty(self):
        """jiuwen.flowMcp 同样置空。"""
        node = _make_node(
    ...
    def test_mcp_no_outputs_still_empty_schema(self):
...
class TestNonMcpOutputsSchema:
    """Plugin、Code、LLM 等节点的 outputs_schema 由 _convert_schema 生成。"""

    def test_plugin_node_outputs_schema_not_empty(self):
        node = _make_node(
    ...
    def test_code_node_outputs_schema(self):
        node = _make_node(
    ...
    def test_llm_node_outputs_schema(self):
        node = _make_node(
...
class TestTimeoutRetry:
    ...
    def test_string_timeout_falls_back_to_default(self):
    ...
    def test_negative_retry_falls_back_to_zero(self):
        node = _make_node(exception_process=_make_ep(retry_times=-1))
        config = _parse_exception_config(node)
        assert config.retry_times == 0

    def test_string_retry_falls_back_to_zero(self):
...
class TestDefaultOutputs:
    def test_default_outputs_passed_through(self):
    ...
    def test_missing_default_outputs_defaults_to_empty(self):
...
class TestNodeType:
    def test_node_type_recorded(self):
    ...
    def test_plugin_node_type_recorded(self):
        node = _make_node(
            node_type="jiuwen.plugin",
            exception_process=_make_ep(),
        )
