...
def _handle_error_bytes(item) -> bytes:
    return asyncio.run(exec_utils._handle_error_response(item))
...
def test_handle_error_response_contains_code_and_safe_message():
    ...
    assert exec_utils._SAFE_PUBLIC_ERROR_MESSAGE.encode() in wire
...
def _drive_process_streaming_output(item) -> list[bytes]:
    ...
    async def collect():
        out = []
        async for chunk in exec_utils._process_streaming_output(origin(), collector=None):
            out.append(chunk)
        return out
...
def test_process_streaming_output_contains_code_and_safe_message():
    ...
    assert exec_utils._SAFE_PUBLIC_ERROR_MESSAGE.encode() in joined
...
class _FakeAESM:
    ...
    def get_state_parsed(self, conv):
...
def _drive_format_component_output(item, monkeypatch):
    ...
    async def collect():
        out = []
        async for chunk in exec_utils._format_component_output(
...
def test_post_process_async_persisted_message_no_sentinel(monkeypatch):
    ...
    assert exec_utils._SAFE_PUBLIC_ERROR_MESSAGE in blob
...
def test_handle_init_exceptions_no_sensitive_in_persisted(monkeypatch):
    ...
    assert _SENTINEL not in blob
    assert async_exec_utils._SAFE_PUBLIC_ERROR_MESSAGE in blob
...
def test_async_execution_safe_message_constant_no_sentinel():
    assert _SENTINEL not in async_exec_utils._SAFE_PUBLIC_ERROR_MESSAGE
...
def test_arun_except_workflow_abort_real_path():
    ...
    exc = _abort_exception({"user_field": "user-value"})
    stub = _arun_stub(exc)
    result = asyncio.run(Workflow._arun(stub, "query", False, {}))
...
def test_get_output_isinstance_workflow_abort_real_path():
    ...
    result = asyncio.run(Workflow._get_output(stub))
    assert result["code"] == StatusCode.WORKFLOW_EXCEPTION_END_ERROR.code
    assert result["message"] == StatusCode.WORKFLOW_EXCEPTION_END_ERROR.errmsg
    assert result["user_field"] == "user-value"
    assert "user-value" not in result["message"]
...
def test_abort_two_real_paths_identical_and_safe():
    exc = _abort_exception({"user_field": "user-value"})
    arun_result = asyncio.run(Workflow._arun(_arun_stub(exc), "q", False, {}))
    getout_result = asyncio.run(Workflow._get_output(_get_output_stub(exc)))
...
def test_abort_two_real_paths_no_log_verbose_dependency(monkeypatch):
    ...
    arun_false = asyncio.run(Workflow._arun(_arun_stub(exc_f), "q", False, {}))
    getout_false = asyncio.run(Workflow._get_output(_get_output_stub(exc_f)))
    monkeypatch.setenv("LOG_VERBOSE", "true")
    exc_t = _abort_exception({"user_field": "v"})
    arun_true = asyncio.run(Workflow._arun(_arun_stub(exc_t), "q", False, {}))
    getout_true = asyncio.run(Workflow._get_output(_get_output_stub(exc_t)))
    # 四格矩阵
...
def test_8_task_done_callback_wraps_non_jiuwen_with_canonical(monkeypatch):
    ...
    async def _run():
        Workflow._task_done_callback(stub, task)
...
def test_9_get_output_jiuwen_branch_chain_to_downstream_no_sentinel(monkeypatch):
    ...
    produced = asyncio.run(Workflow._get_output(stub))
...
def test_8_9_chain_two_configs_identical(monkeypatch):
    ...
    produced_false = asyncio.run(Workflow._get_output(_get_output_stub(exc)))
    monkeypatch.setenv("LOG_VERBOSE", "true")
    produced_true = asyncio.run(Workflow._get_output(_get_output_stub(exc)))
...
def test_normal_end_no_error_frame_two_configs(monkeypatch, caplog):
...
def test_message_end_no_error_frame_two_configs(monkeypatch, caplog):
...
def test_cancel_propagates_no_synthetic_error_two_configs(monkeypatch, caplog):
