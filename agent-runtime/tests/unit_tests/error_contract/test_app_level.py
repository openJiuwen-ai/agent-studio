"""COM-03 §10.4: app 级 HTTPException 出口测试。

回归防护：HTTPException handler 必须注册到 starlette 父类——路由级
404/405 抛 starlette.exceptions.HTTPException（父类），若只注册
fastapi.HTTPException（子类），父类异常会落回 FastAPI 默认
``{"detail":...}`` 出口（2026-09-14 live 冒烟发现）。
"""
...
def test_route_405_returns_method_not_allowed():
    ...
    assert r.json()["error_code"] == "openjiuwen.12100003"


def test_fastapi_raised_http_exception_also_caught_by_parent_handler():
    """fastapi.HTTPException（子类实例）同样命中父类注册的 handler。"""
    ...
    assert r.json()["error_code"] == "openjiuwen.12100001"
