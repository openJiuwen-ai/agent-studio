...
class TestStreamableHttpClientNewConstruction:
    def test_basic_construction(self):
        config = _make_config()
        client = StreamableHttpClientNew(config)
    ...
    def test_client_name_attribute(self):
        config = _make_config()
        client = StreamableHttpClientNew(config)
        assert client.__client_name__ == "streamable_http_new"
...
class TestPrepareRequestParams:
    def test_basic_prepare(self):
        ...
        client = StreamableHttpClientNew(config)
        with patch(
            "jiuwen.extension.wrapper.streamable_http_client_new.get_x_request_id",
            return_value="req-123",
        ):
            with patch(
                "jiuwen.extension.wrapper.streamable_http_client_new.get_x_execution_id",
...
class TestExecuteAuthHook:
    ...
    def test_hook_function_not_found(self):
        ...
        rp.query_params_in_inputs = {}
        with patch(
            "jiuwen.extension.wrapper.streamable_http_client_new.ApiUtils.get_hook_func",
            return_value=None,
        ):
            client._execute_auth_hook(rp, "test_tool")
...
class TestListTools:
    ...
    async def test_list_tools_success(self):
        ...
        with patch(
            ...
            with patch(
                "jiuwen.extension.wrapper.streamable_http_client_new.ClientSession",
    ...
    async def test_list_tools_exception_group(self):
        ...
        with patch(
            ...
            mock_client.return_value.__aenter__ = AsyncMock(side_effect=eg)
            mock_client.return_value.__aexit__ = AsyncMock(return_value=None)
            tools = await client.list_tools()
            assert tools == []
