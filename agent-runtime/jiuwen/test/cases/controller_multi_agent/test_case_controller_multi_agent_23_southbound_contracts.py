...
from jiuwen.test.cases.controller_multi_agent.test_case_controller_multi_agent_23 import (
...
ROUTABLE_WORKFLOW_IDS = {START_WORKFLOW_ID, SERVICE_WORKFLOW_ID, DEFAULT_WORKFLOW_ID}
...
def local_ir_path(tmp_path_factory, monkeypatch):
    ...
    tmp_path = tmp_path_factory.mktemp("controller_multi_agent_case23_southbound")
    return _copy_case_ir_bundle(tmp_path)


def test_case23_modelnew_records_routing_inputs_from_real_ir(local_ir_path):
    ...
    assert "test query 23" in _extract_model_prompt_text(relevant_calls[0])
...
def test_case23_workflownew_astream_complete_contract():
    workflow_runtime = _RecordingWorkflowRuntime(
        {
            DEFAULT_WORKFLOW_ID: [
                [
    ...
    stream = asyncio.run(
        workflow.astream(
            query="test query 23",
            params={
                "conversation_history": [{"role": "user", "content": "test query 23"}]
            },
            workflow_id=DEFAULT_WORKFLOW_ID,
    ...
    assert workflow_runtime.calls[0]["query"] == "test query 23"
...
def test_case23_workflownew_resume_contract():
    workflow_runtime = _RecordingWorkflowRuntime(
        {
            END_WORKFLOW_ID: [
                [
    ...
    second_stream = asyncio.run(
        workflow.astream(
            query="resume input 23",
            params={
                "conversation_history": [{"role": "user", "content": "resume input 23"}]
            },
            workflow_id=END_WORKFLOW_ID,
            agent_id=CHILD_AGENT_ID,
            session_id="conversation_resume",
    ...
    assert workflow_runtime.calls[1]["query"] == "resume input 23"
    assert (
