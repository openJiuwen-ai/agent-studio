...
ROUTABLE_WORKFLOW_IDS = {
    ...
    "33d0e157-318d-4b35-96a3-0674a48c9a9f",
}
...
def local_ir_path(tmp_path_factory, monkeypatch):
    ...
    tmp_path = tmp_path_factory.mktemp("controller_multi_agent_case01_southbound")
    return _copy_case_ir_bundle(tmp_path)


def test_case01_modelnew_records_routing_inputs_from_real_ir(local_ir_path):
    ...
    workflow_runtime = _RecordingWorkflowRuntime(
        {
            ...
            "33d0e157-318d-4b35-96a3-0674a48c9a9f": [
                [
                    _build_message_end_stream_data(
                        ...
                        answer="second workflow answer",
                        node_id="node_message_2",
                        node_name="node_message_2",
                        node_type="jiuwen.message",
                    ),
                    _build_end_stream_data(
                        workflow_id="33d0e157-318d-4b35-96a3-0674a48c9a9f",
            ...
            "8f9ac059-0b15-4b11-baf2-18d10665c38c": [
                [
                    _build_message_end_stream_data(
    ...
    assert "test query" in first_prompt
    assert len(workflow_runtime.calls) >= 1 or "task_end" in response_text
    if workflow_runtime.calls:
        assert workflow_runtime.calls[0]["query"] == "test query"
        assert workflow_runtime.calls[0]["workflow_id"] in ROUTABLE_WORKFLOW_IDS
    assert '"result": 2' not in response_text
...
def test_case01_workflownew_astream_complete_contract():
    workflow_runtime = _RecordingWorkflowRuntime(
        {
            "32964224-6644-4483-94ae-40d1c8532c52": [
                [
    ...
    stream = asyncio.run(
        workflow.astream(
            query="test query",
            params={
                "conversation_history": [{"role": "user", "content": "test query"}]
            },
            workflow_id="32964224-6644-4483-94ae-40d1c8532c52",
            agent_id="child-agent-鐞嗚储鏈嶅姟",
    ...
    assert workflow_runtime.calls[0]["agent_id"] == "child-agent-鐞嗚储鏈嶅姟"
    assert events[0].code == StreamCode.MESSAGE_END.value
    assert events[1].code == StreamCode.FINISH.value


def test_case01_workflownew_resume_contract():
    workflow_runtime = _RecordingWorkflowRuntime(
        {
            "8f9ac059-0b15-4b11-baf2-18d10665c38c": [
                [
    ...
    second_stream = asyncio.run(
        workflow.astream(
            query="resume query",
            params={
                "conversation_history": [{"role": "user", "content": "resume query"}]
            },
            workflow_id="8f9ac059-0b15-4b11-baf2-18d10665c38c",
            agent_id="child-agent-鐞嗚储鏈嶅姟",
            session_id="conversation_resume",
    ...
    assert (
        workflow_runtime.calls[1]["workflow_id"]
        == "8f9ac059-0b15-4b11-baf2-18d10665c38c"
    )
    assert workflow_runtime.calls[1]["query"] == "resume query"
    assert (
