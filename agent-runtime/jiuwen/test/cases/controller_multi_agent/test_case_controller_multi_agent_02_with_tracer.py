...
基于 test_case_controller_multi_agent_02 的用例，增加 openjiuwen Tracer 验证。
...
def _make_fake_redis_client() -> MagicMock:
    ...
    client.rpush = AsyncMock(return_value=None)
...
def local_ir_path(tmp_path_factory, monkeypatch):
    ...
    tmp_path = tmp_path_factory.mktemp("controller_multi_agent_case02_tracer")
...
class TestTracerE2EMultiTurn:
    ...
    def test_multi_turn_tracer_continuity(self, local_ir_path):
        ...
        Turn 2: "是本人,我要赎回理财" → 工作流恢复
        Turn 3: "第一笔，全部赎回。" → 工作流继续
        Turn 4: "今天天气怎么样" → 默认工作流
        """
        tracer, emitter = _create_initialized_tracer()

        cases = _load_cases()
        conversation_id = "conversation_controller_multi_agent_02_tracer_e2e"
    ...
    def test_root_span_is_first(self, local_ir_path):
        ...
        if payloads:
            ...
            assert first["invokeType"] in root_invoke_types, (
                f"First payload should be a root span, got invokeType={first['invokeType']}"
            )
