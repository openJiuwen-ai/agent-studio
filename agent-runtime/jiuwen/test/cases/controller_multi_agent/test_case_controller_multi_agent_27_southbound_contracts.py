...
FAQ_WORKFLOW_ID = "aacb7592-31ff-4741-a079-cfc187cabff8"
START_WORKFLOW_ID = "32964224-6644-4483-94ae-40d1c8532c52"
...
def local_ir_path(tmp_path_factory, monkeypatch):
    ...
    tmp_path = tmp_path_factory.mktemp("controller_multi_agent_case27_southbound")
    return _copy_case_ir_bundle(tmp_path)


def test_case27_modelnew_records_routing_inputs_from_real_ir(local_ir_path):
            ...
            FAQ_WORKFLOW_ID: [
                [
                    _build_message_end_stream_data(
                        workflow_id=FAQ_WORKFLOW_ID,
                        answer="faq workflow answer",
                        node_id="node_faq_message",
                        node_name="node_faq_message",
                        node_type="jiuwen.message",
                    ),
                    _build_end_stream_data(
                        workflow_id=FAQ_WORKFLOW_ID, answer="faq workflow answer"
                    ),
                ]
            ],
            START_WORKFLOW_ID: [
                [
                    ...
                    _build_end_stream_data(
                        workflow_id=START_WORKFLOW_ID, answer="start workflow answer"
                    ),
                ]
            ],
            SERVICE_WORKFLOW_ID: [
            ...
            END_WORKFLOW_ID: [
                [
                    ...
                    _build_questioner_finish_stream_data(
                        workflow_id=END_WORKFLOW_ID, answer=""
                    ),
                ]
            ],
        }
    ...
    assert "test query 27" in _extract_model_prompt_text(relevant_calls[0])
...
def test_case27_workflownew_astream_complete_contract():
    workflow_runtime = _RecordingWorkflowRuntime(
        {
            FAQ_WORKFLOW_ID: [
                [
    ...
    stream = asyncio.run(
        workflow.astream(
            query="test query 27",
            params={
                "conversation_history": [{"role": "user", "content": "test query 27"}]
            },
            workflow_id=FAQ_WORKFLOW_ID,
    ...
    assert workflow_runtime.calls[0]["query"] == "test query 27"
    assert workflow_runtime.calls[0]["workflow_id"] == FAQ_WORKFLOW_ID
...
def test_case27_workflownew_resume_contract():
    workflow_runtime = _RecordingWorkflowRuntime(
        {
            END_WORKFLOW_ID: [
                [
                ...
                [
                    ...
                    _build_end_stream_data(
                        workflow_id=END_WORKFLOW_ID, answer="resume workflow answer"
                    ),
                ],
            ],
        }
    ...
    second_stream = asyncio.run(
        workflow.astream(
            query="resume input 27",
            params={
                "conversation_history": [{"role": "user", "content": "resume input 27"}]
            },
            workflow_id=END_WORKFLOW_ID,
            agent_id=CHILD_AGENT_ID,
            session_id="conversation_resume",
    ...
    assert workflow_runtime.calls[1]["query"] == "resume input 27"
    assert (
