...
def _is_llm_call_logging_enabled() -> bool:
    """检查 WORKFLOW_LOG_LEVEL 是否 ≤ INFO（仅控制 input/output 回调）。

    INFO 及更细级别下注册 input/output 回调，使模型请求体/响应等 INFO 日志默认输出；
    WARNING/ERROR 级别不注册，省掉回调体里 json.dumps 的开销。
    错误回调不受此门控约束，始终注册。
...
def register_llm_call_logging_callbacks() -> None:
    ...
    - LLM_CALL_ERROR：无条件注册。错误回调仅在调用失败时触发，成功路径零开销；
      错误是低频高重要事件，应始终可见，不受 WORKFLOW_LOG_LEVEL 门控。
    - LLM_INPUT / LLM_OUTPUT：仅当 WORKFLOW_LOG_LEVEL ≤ INFO 时注册。
      这两个回调每次模型调用都触发，门控用于在 WARNING/ERROR 级别下省掉
      回调体里 json.dumps(完整 messages) 的开销。
