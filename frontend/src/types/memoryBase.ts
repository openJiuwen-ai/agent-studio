...
export interface MemoryBaseDocument {
  id: string;
  name: string;
  memoryBaseId: string;
...
export interface UpdateMemoryBaseRequest {
  ...
  name: string;
  description?: string;
  llm_model_config_id?: number; // 可选更新字段
}
...
export interface MemoryBaseSearchResponse {
...
export interface MemoryBaseStatistics {
  totalMemoryBases: number;
  activeMemoryBases: number;
  totalMemories: number;
  recentlyCreated: MemoryBase[];
  recentlyUpdated: MemoryBase[];
}
