  ...
  return (
    <BasePickerContainer onClose={onClose} position={position}>
      ...
      <div className="px-3 py-2 border-b border-gray-100 dark:border-gray-700">
        <span className="text-xs font-medium text-gray-500 dark:text-gray-400">{t('apps.model.selectModel')}</span>
      </div>

      {/* 温馨提示 */}
      <div className="mx-3 mt-2 mb-1 px-2.5 py-1.5 rounded-md border border-amber-200 dark:border-amber-800 bg-amber-50 dark:bg-amber-900/20 flex items-start gap-2">
      ...
      <div className="py-1">
        {models.map((model, index) => {
          ...
          return (
            ...
            >
              {/* 图标 */}
              <div className={`flex-shrink-0 p-1 ${RADIUS_SMALL} text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30`}>
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center justify-between">
                  ...
                  {(isSelected || model === selectedModel) && <Check className="w-3 h-3 text-blue-600 dark:text-blue-400" />}
                </div>
              </div>
            </div>
          )
