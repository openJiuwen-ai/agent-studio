...
const CustomTextarea = ({
  ...
  <div>
    ...
    <textarea
      ...
      placeholder={placeholder}
      rows={rows}
      className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:outline-none dark:bg-gray-800 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-transparent"
      {...props}
    />
  </div>
)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!memoryBase) {
      ...
      if (embeddingModels.length === 0) {
        ...
        const selectedEmbeddingModel = embeddingModels.find(model => parseInt(model.id) === formData.embedding_model_config_id)
      ...
      if (modelsList.length === 0) {
        ...
        const selectedLlmModel = modelsList.find(model => model.model_id === formData.llm_model_config_id)
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto">
      <div className="flex items-center justify-center min-h-screen px-4">
        ...
        <div className="relative bg-white dark:bg-gray-900 rounded-lg max-w-2xl w-full border dark:border-gray-700">
          ...
          <div className="p-6">
            <p className="text-gray-600 dark:text-gray-400 mb-6">{memoryBase ? t('memoryBases.edit.description') : t('memoryBases.create.description')}</p>
            ...
            <form onSubmit={handleSubmit}>
              <div className="space-y-4">
                ...
                {!memoryBase && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                      {t('memoryBases.form.embeddingModelRequired')}{' '}
                    ...
                    {isLoadingEmbeddingModels ? (
                      ...
                      <div className="flex flex-col gap-2">
                        <p className="text-sm text-red-500">{t('memoryBases.form.noModels')} </p>
                        <Link
                          to="/dashboard/models"
                ...
                {!memoryBase && (
                  <div>
                    <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">
                      {t('memoryBases.form.llmModelRequired')}{' '}
                      <span className="text-red-500 dark:text-red-400">
                        *
                    ...
                    {isLoadingLlmModels ? (
                      ...
                      <div className="flex flex-col gap-2">
                        <p className="text-sm text-red-500">{t('memoryBases.form.noLlmModels')} </p>
                        <Link
                          to="/dashboard/models"
                          className="inline-flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium text-white bg-blue-500 rounded-lg hover:bg-blue-600 transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2"
                        >
                    ...
                    {errors.llm_model_config_id && <p className="text-red-500 text-sm mt-1">{errors.llm_model_config_id}</p>}
                  </div>
                )}
              </div>
              ...
              <div className="mt-6 pt-6 border-t dark:border-gray-700">
                {isAtLimit && <p className="text-sm text-red-500 mb-3">{t('memoryBases.form.limitReached')}</p>}
