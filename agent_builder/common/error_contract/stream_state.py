"""COM-03 §6.2 + COM-07A §4.2: 流式状态与唯一终态守卫。

流状态：NOT_STARTED → STREAMING → ENDED / TERMINATED / CANCELLED。

规则：
- NOT_STARTED 失败：走普通 HTTP 构建器，不产生 SSE 帧；
- STREAMING 正常结束：原子切换为 ENDED，最多写一个 END；
- STREAMING 失败：原子切换为 TERMINATED，最多写一个 SSE error；
- error / END / cancel 互斥，写出后禁止继续发送 message、success end 或 done；
...
class SseTerminalGuard:
    ...
    def try_end(self) -> bool:
        """COM-07A §4.2: 原子状态迁移 STREAMING → ENDED（成功终态）。

        只有赢家（返回 True）允许写 END 帧；error / END / cancel 互斥。
        """
        with self._lock:
    ...
    def cancel(self) -> None:
        """客户端取消/正常断开。转为 CANCELLED，不发 error/END。"""
        with self._lock:
            if self._state not in (StreamState.ENDED, StreamState.TERMINATED):
