...
function main() {
  ...
  log "[7/7] 打包所有镜像为 AgentBuilder-arm64.tar.gz"
  docker_save_package
  log "[7/7] 打包完成"
...
# docker save studio-builder 为独立 StudioBuilder-arm64.tar.gz
function docker_save_builder() {
  cd ${DOCKER_DIR}
  if [ -f "StudioBuilder-arm64.tar.gz" ]; then
    rm -f "StudioBuilder-arm64.tar.gz"
  fi
  docker save studio-builder:${VERSION}.${BUILD_TIME}.${BUILD_PLATFORM} | gzip > StudioBuilder-arm64.tar.gz
}
